...
![系统总览](docs/images/02-system-overview.png)
...
![Agent Skill](docs/images/09-agent-skills.png)
...
![SKILL.md 编辑](docs/images/18-skill-editor-skill-md.png)

`SKILL.md` 应该保持小而稳定，负责说明什么时候使用这个 Skill、输入输出是什么、安全边界是什么，以及应该继续读取哪些文件。

#### 流程、参考和示例

![Workflows](docs/images/19-skill-editor-workflows.png)

![References](docs/images/20-skill-editor-references.png)

![Examples](docs/images/21-skill-editor-examples.png)
...
![Skill 发布](docs/images/23-skill-editor-publish.png)
...
![Token 权限](docs/images/25-token-permissions.png)
...
![Agent ZIP 备份](docs/images/27-agent-backup-zip.png)

ZIP 备份适合完整迁移和长期归档。它会包含 Agent 身份、挂载关系、Skill 文件内容、记忆和目标。

![Agent 导入](docs/images/28-agent-import.png)

导入后可以在新环境中恢复 Agent 资产，适合克隆、恢复或跨机器迁移。
...
![用户管理](docs/images/30-admin-users.png)

管理员可以管理用户、角色和账号状态。

![Skill 审核](docs/images/31-admin-skills.png)

Skill 发布可以进入审核流程，管理员可以通过、拒绝、下架或重新发布。

![反馈管理](docs/images/32-admin-feedback.png)

反馈管理用于收集用户问题、建议和 NPS 类评价。

![公告管理](docs/images/33-admin-announcements.png)
...
![同步流程](docs/images/36-sync-flow.png)
...
![技术架构](docs/images/37-architecture.png)
...
- `05-agent-dashboard.png`
- `07-agent-detail-overview.png`
- `15-skill-editor-overview.png`
- `17-skill-editor-file-tree.png`
- `25-token-permissions.png`
- `26-transfer-overview.png`
- `30-admin-users.png`
- `36-sync-flow.png`
- `37-architecture.png`
...
- 桌面端使用 `1440 x 900` 左右。
- 移动端使用 `390 x 844` 左右。
- Token、邮箱、服务器 IP、密钥、真实项目记忆必须打码。
- 使用演示数据，不要截图真实敏感内容。
- 完整截图清单见：[docs/gallery.md](docs/gallery.md)。
