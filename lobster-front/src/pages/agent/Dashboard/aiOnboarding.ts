...
export const buildAiAgentOnboardingPrompt = ({
  ...
  return [
    ...
    '5. 同步技能快照、记忆或目标前，先调用 GET $LOBSTER_API_BASE_URL/api/ai/agents/{agentId}/config?brief=true 拉取平台现有配置，读取 data.syncRevision，对比已有 skills、memories、goals 和 skillPackages。',
    ...
    '9. 有 pending 事件时，调用 GET $LOBSTER_API_BASE_URL/api/ai/agents/{agentId}/config?brief=true；brief 只含已挂载 Skill 元数据，platformSkillCount 是技能库总数，mountedSkillPackageCount 是当前 Agent 挂载数。',
    '10. 根据事件 payload 或 brief 差异按需调用 GET /api/ai/agents/{agentId}/memories/{memoryId} 或 GET /api/ai/skills/{idOrCode}/detail。',
    '11. 确认配置已应用后，再调用 POST $LOBSTER_API_BASE_URL/api/ai/events/{eventId}/ack。',
    '',
