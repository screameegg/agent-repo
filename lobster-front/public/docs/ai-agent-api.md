...
如果要新建一个面向平台复用的 Skill Package，可以按上面的结构组织内容。但本地 Agent 同步已有 Skill 时，不要要求用户手动拆分。只要本地已经存在 `SKILL.md`、`workflows/`、`references/`、`examples/`、`checklists/` 等目录，本地 Agent 可以直接上传本地 Skill 目录的完整文件树。
...
Agent 第一次接入、迁移 Skill 或准备长期运行时，必须先让用户选择同步策略。不要默认替用户开启后台写入，也不要把一次性同步误写成长期任务。

建议提供这些选项：

| 选项 | 适用场景 | AI 应执行的动作 |
| --- | --- | --- |
| 手动预览同步 | 默认推荐。用户只想本次上传 Skill、记忆或目标 | 生成同步预览，让用户选择范围；确认后只执行本次 `/sync` 或 Skill Package 上传，不创建后台任务。 |
| 强定时同步 | 用户明确希望 Agent 长期按周期同步平台状态 | 引导用户确认周期、同步范围、失败退避和日志位置；写入定时任务或运行脚本，定时调用 `events`、`config?brief=true`、必要的 detail 接口和 `/sync`。 |
| 强一致同步 | 用户要求平台尽量实时反映 Agent 执行状态 | 在每个可审计任务完成后同步到平台，例如更新 `goals[].steps`、新增/更新关键记忆或刷新技能快照；每次写入前仍要使用最新 `syncRevision`，发生 `409` 时重新拉取配置并重试或要求用户确认。 |

选择规则：

- 如果用户没有明确选择，使用“手动预览同步”。
- 如果用户选择“强定时同步”，AI 必须说明计划写入的定时任务、触发频率、同步范围和停止方式，并在用户确认后再创建任务。
- 如果用户选择“强一致同步”，AI 必须把每个任务的完成状态同步到平台；长期目标应复用已有 `goals[].id` 和 `steps[].id`，不要重复创建同名目标。
- 强定时和强一致都不能绕过同步预览、`baseRevision`、`confirmSync=true` 和 409 冲突处理。
- 后台任务写日志时不要输出完整 Agent Token，只记录请求路径、响应 `code`、资源数量和脱敏后的 Agent/Skill 标识。

## 平台迁移策略

不同 AI 平台迁移 Skill 的方式不完全一样，不要把一种平台的读取方式硬套到所有平台：

- Codex：通常可以读取工作区文件、运行测试和编辑代码。迁移知栈 Skill 时，优先写程序或脚本读取本地 Skill 目录，按现有 `SKILL.md`、`workflows/`、`references/`、`examples/`、`checklists/` 生成完整 Skill 文件树和同步预览，再按用户选择提交；不要要求用户手动拆分。
...
5. 同步 `skills`、`memories` 或 `goals` 前，必须调用 `GET /api/ai/agents/{agentId}/config?brief=true` 拉取线上配置，读取 `data.syncRevision`，生成同步预览，并让用户选择同步策略和同步范围：手动预览同步、强定时同步、强一致同步、全量同步、只同步 Skill、只同步记忆、只同步目标或取消。
...
`/sync` 是写入接口，默认只返回同步摘要，不返回完整 config：
...
```bash
curl -X POST "$LOBSTER_API_BASE_URL/api/ai/agents/$LOBSTER_AGENT_ID/sync?return=config" \
  -H "Authorization: Bearer $LOBSTER_AGENT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name":"Local Agent"}'
```
...
- `steps[].status` 支持：`pending`、`running`、`in_progress`、`completed`、`skipped`、`failed`。
...
本地 Agent 可以直接上传本地 Skill 目录的完整文件树。读取本地 `SKILL.md` 以及同级 `workflows/`、`references/`、`examples/`、`checklists/`、`scripts/` 后，按 `path` 组装 `files` 数组提交即可，不要要求用户手动拆分。

如果是从一整份长文档新建平台 Skill Package，才建议整理成“入口 + 流程 + 参考 + 样例 + 检查”的文件树：
...
只需要判断 revision 和数量时，可以使用更轻的范围：

```bash
curl -X GET "$LOBSTER_API_BASE_URL/api/ai/agents/$LOBSTER_AGENT_ID/config?scope=revision" \
  -H "Authorization: Bearer $LOBSTER_AGENT_TOKEN"
```
