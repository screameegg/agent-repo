...
- Agent 学习已挂载 Skill 时，读取 `config.data.skillPackages[].files`，不要只读取 `config.data.skills`；`skills` 只是展示快照。
- 业务错误要检查响应体 `code`，不能只看 HTTP 状态。
...
2. 周期性调用 `GET /api/ai/agents/{agentId}/events`。发现 pending 的 `config_changed` 后，先调用 `GET /api/ai/agents/{agentId}/config`，不要先 ack。
3. 对每个 `skillPackages[]` 按 `code` 建立本地缓存。相同 `code` 的新返回覆盖旧缓存，不要追加成重复 Skill。
4. 按 `skillPackages[].files[].path` 还原目录结构。`path` 使用 `/`，根目录必须优先寻找 `SKILL.md`。
5. 先读每个 Skill 的根 `SKILL.md`。`SKILL.md` 应该是触发条件、边界和文件索引，不应该是巨型全量手册。
6. 根据 `SKILL.md` 的索引按需读取 `workflows/`、`references/`、`examples/`、`checklists/`。当前任务用不到的长文档不要提前塞进上下文。
7. `scripts/` 只能当作可选工具或示例。只有 Skill 明确要求、用户/运行环境允许、且沙箱策略允许时才能执行。
8. Agent 确认已经应用平台侧配置变更后，再调用 `POST /api/ai/events/{eventId}/ack`。ack 后该事件不会再次出现在 events 里。
...
Agent 在提交包含 `skills`、`memories`、`goals` 的 `/sync` 请求，或更新 Skill Package 的完整 `files` 前，必须先生成同步预览。同步预览要把本地准备写入的数据和 `GET /api/ai/agents/{agentId}/config` 返回的线上数据放在一起比较，并展示这些信息：
...
5. 同步 `skills`、`memories` 或 `goals` 前，必须调用 `GET /api/ai/agents/{agentId}/config` 拉取线上配置，读取 `data.syncRevision`，生成同步预览，并让用户选择全量同步、只同步 Skill、只同步记忆、只同步目标或取消。
...
11. 有 pending 的 `config_changed` 时，调用 `GET /api/ai/agents/{agentId}/config`。
12. Agent 按 `skillPackages[].code` 覆盖本地缓存，按 `skillPackages[].files[].path` 还原文件树，先读根 `SKILL.md`，再按索引按需读取 `workflows/`、`references/`、`examples/`、`checklists/`；`scripts/` 默认不执行。
13. 确认已应用平台侧配置变更后，调用 `POST /api/ai/events/{eventId}/ack`。
...
11. 如果返回 pending 的 `config_changed`，拉取完整配置并读取 `skillPackages[].files`；这是 AI 获取平台挂载 Skill 文件树的入口。
  ...
  -d '{
    ...
    "files": [
      ...
      },
      {
        ...
        "language": "markdown",
        "content": "# 配置监听\n\n1. 发现 config_changed 后先拉取 config。\n2. 按 skillPackages[].files 重建文件树。\n3. 应用完成后再 ack。\n"
      },
      {
        "nodeType": "file",
        "name": "interface.md",
...
1. 拉取 events。
2. 如果存在 pending 的 `config_changed`，调用 config 拉取完整配置。
3. Agent 按 `skillPackages[].files` 重建并重载 Skill 文件树，再应用平台侧配置变更。
4. 对已处理事件逐条 ack。
...
- `skillPackages`：读取已挂载 Skill 的完整文件树。学习平台搭配技能时优先处理这里的 `files`。
- `memories`：合并到长期记忆。
- `goals`：更新发展方向和执行任务。目标可以长期存在，AI 应复用 `goals[].id` 持续同步 `steps`，不需要填写整体百分比。

Skill 文件树处理规则：

- 按 `skillPackages[].files[].path` 还原目录结构。
- 优先读取根目录 `SKILL.md`。
...
如果返回里存在 pending 的 `config_changed`，拉取配置：
...
Agent 必须先重建并应用 `skillPackages[].files` 文件树，再 ack；不要把自己的 `/sync` 上传结果当成需要 ack 的配置事件：
...
- `409`：同步前未拉取平台配置、未带 `baseRevision`、未设置 `confirmSync=true`，或平台配置已在拉取后变化。重新调用 `GET /api/ai/agents/{agentId}/config`，对比 `skills`、`memories`、`goals`、`skillPackages` 后再提交。
